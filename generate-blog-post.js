/**
 * generate-blog-post.js — GlobalCampaignHub
 * ─────────────────────────────────────────────────────
 ...
 * 7. Deep linking: keyword maps to exact merchant LC affiliate URL
 ...
 * RUN with keyword: node generate-blog-post.js --keyword "nordvpn review"
 * RUN with lang batch: node generate-blog-post.js --lang en,es,fr,de
 */
...
// ── Load keywords-1000.json (flat array format) ────────────────────────────
const RAW_KEYWORDS = JSON.parse(readFileSync(resolve(__dir, 'keywords-1000.json'), 'utf-8'));

// Build affiliate map: merchant -> LC url
// Format: https://www.linkconnector.com/ta.php?lc=LCCODE&atid=ATID
// We derive atid from the merchant slug (strip gh- prefix)
function buildAffUrl(lc, merchant) {
  const atid = merchant.replace(/^gh-/, '');
  return `https://www.linkconnector.com/ta.php?lc=${lc}&atid=${atid}`;
}

// Build unique merchant LC map from keywords (use first occurrence per merchant)
const MERCHANT_LC = {};
...
// 100 languages (same list as other sites)
const LANG_LIST = [
...
// ── GitHub Models API call with retry ──────────────────────────────────────
...
function pickKeywords(manualKeyword) {
  if (manualKeyword) {
    const found = RAW_KEYWORDS.find(k =>
  ...
  // Sort by volume desc (already sorted in file, but enforce)
...
function getAffUrl(kw) {
  return buildAffUrl(kw.lc, kw.merchant);
}

// ── Get related merchants for cross-linking ────────────────────────────────
function getRelatedMerchants(kw, limit = 4) {
  return Object.entries(MERCHANT_LC)
    .filter(([id]) => id !== kw.merchant)
    .slice(0, limit)
    .map(([id, data]) => ({ id: id.replace(/^gh-/, ''), url: data.url }));
}

// ── Write blog post via AI ─────────────────────────────────────────────────
async function writeBlogPost(kw) {
  ...
  // Get all keywords for this merchant for meta
  const metaKeywords = RAW_KEYWORDS
    .filter(k => k.merchant === kw.merchant)
    .slice(0, 15)
    .map(k => k.keyword)
    .join(', ');

  const systemPrompt = `You are a senior SEO content writer and affiliate marketing expert for GlobalCampaignHub. You write high-quality, genuinely helpful blog posts that rank on Google and drive affiliate sales through LinkConnector. You never write thin content. Every post is comprehensive, detailed, and provides real value.`;
...
- Use varied anchor text (not always the same phrase)
...
OUTPUT FORMAT — return valid JSON only, no markdown fences:
{
  ...
  "metaDescription": "Meta description (150-160 chars, keyword + CTA)",
  "metaKeywords": "${metaKeywords}",
  "slug": "${kw.slug || slugify(kw.keyword)}",
  "h1": "H1 heading (slightly different from title)",
  "intro": "3-4 paragraph intro (400+ words). Affiliate link naturally in paragraph 2.",
  "sections": [
    {
      "h2": "Section heading",
      "content": "Section content (300+ words). Affiliate link + specific examples. End with: <a href='${affUrl}' class='cta-btn' target='_blank' rel='sponsored'>Visit ${merchantName} →</a>",
      "hasCTA": true
    }
  ],
  "comparisonTable": "HTML table comparing this merchant vs 2-3 competitors. Affiliate link in winner column.",
  "pros_cons": {
    "h2": "Pros and Cons of ${merchantName}",
    "pros": ["pro 1", "pro 2", "pro 3"],
    "cons": ["con 1", "con 2"]
  },
  "faq": [
    { "q": "FAQ question", "a": "Detailed answer (100+ words). Affiliate link where natural." }
  ],
  "conclusion": "2-3 paragraph conclusion. Affiliate link twice. Strong CTA at end.",
...
- Include real product names, price ranges, and use cases
...
- The post must genuinely help someone searching for "${kw.keyword}"`;
  ...
  post.merchant     = kw.merchant;
  post.merchantName = merchantName;
  post.affUrl       = affUrl;
...
function buildHTML(post, lang, slug) {
  ...
  const wordCount = post.wordCount || 1800;
  ...
  const sectionsHTML = (post.sections || []).map(s => `
    <section class="post-section">
      <h2>${s.h2 || ''}</h2>
      <div class="section-content">${String(s.content || '')}</div>
    </section>`
  ).join('\n');

  // pros/cons HTML
  const prosConsHTML = post.pros_cons ? `
    <section class="post-section">
      <h2>${post.pros_cons.h2 || 'Pros and Cons'}</h2>
      <div class="pros-cons-grid">
        <div class="pros-col">
          <h3>✅ Pros</h3>
          <ul>${(post.pros_cons.pros || []).map(p => `<li>${p}</li>`).join('')}</ul>
        </div>
        <div class="cons-col">
          <h3>❌ Cons</h3>
          <ul>${(post.pros_cons.cons || []).map(c => `<li>${c}</li>`).join('')}</ul>
        </div>
      </div>
    </section>` : '';
  ...
  const jsonLD = JSON.stringify({
    ...
    "datePublished": TODAY,
    "dateModified": TODAY,
    "author": { "@type": "Organization", "name": "GlobalCampaignHub" },
...
<head>
  ...
  <style>
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .pros-col h3{color:#47ffb8;margin-bottom:12px;font-family:var(--font-head)}
    .cons-col h3{color:#ff6b6b;margin-bottom:12px;font-family:var(--font-head)}
    .pros-col ul,.cons-col ul{list-style:none;display:flex;flex-direction:column;gap:8px}
    .pros-col li::before{content:"✓ ";color:#47ffb8}
    .cons-col li::before{content:"✗ ";color:#ff6b6b}
...
<body>
  ...
  <div class="container">
    <nav class="breadcrumb" aria-label="Breadcrumb">
      ...
      <a href="${BASE_URL}/blog/${lang}/">Blog</a> ›
      ${post.title}
    </nav>
    ...
    <article itemscope itemtype="https://schema.org/BlogPosting">
      <header class="post-header">
        <div class="post-meta">
          <span>📅 ${TODAY}</span>
          <span>📂 ${post.cat || post.merchantName}</span>
          <span>🌐 ${langMeta.name}</span>
          <span>⏱ ${readTime} min read</span>
        </div>
        <div class="star-row">
          <span class="stars">★★★★★</span>
          <span>4.7/5 · ${kw ? kw.volume.toLocaleString() : ''} monthly searches</span>
        </div>
      ...
      <div class="affiliate-cta-hero">
        ...
        <a href="${affUrl}" class="cta-btn" target="_blank" rel="sponsored noopener">
          Visit ${post.merchantName} — Best Deal Available →
        </a>
      </div>
      ...
      <section class="post-section">
        <h2>How ${post.merchantName} Compares</h2>
        ${post.comparisonTable}
      </section>` : ''}
      ...
      <div class="affiliate-cta-hero" style="margin:48px 0">
        ...
        <a href="${affUrl}" class="cta-btn" target="_blank" rel="sponsored noopener">
          Get the Best Deal at ${post.merchantName} →
        </a>
      </div>
      ...
      <div class="related-merchants">
        ...
        <div class="related-links">
          ${Object.entries(MERCHANT_LC).filter(([id]) => id !== post.merchant).slice(0, 6).map(([id, data]) =>
            `<a href="${data.url}" class="related-link" target="_blank" rel="sponsored noopener">${id.replace(/^gh-/,'')} →</a>`
          ).join('')}
        </div>
      </div>
  ...
  <footer>
    <p class="footer-note">© ${YEAR} GlobalCampaignHub · <a href="${BASE_URL}/">Browse All Campaigns</a> · Affiliate links verified through LinkConnector.</p>
  </footer>
  ...
  <div class="sticky-cta">
    <a class="sticky-btn" href="${affUrl}" target="_blank" rel="sponsored noopener">
      🔗 Best Deal at ${post.merchantName} →
    </a>
  </div>
...
async function translatePost(post, targetLang, targetLangName) {
  const systemPrompt = `You are a professional translator. Translate the provided JSON blog post fields into ${targetLangName}. Preserve ALL HTML tags, affiliate links, and URLs exactly as-is. Return valid JSON only, no markdown.`;
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Blog — GlobalCampaignHub (${langMeta.name})</title>
  <meta name="description" content="Explore merchant reviews, affiliate deals, and buying guides from GlobalCampaignHub." />
  ...
  <style>
    ...
    .card-cta{color:#47ffb8;font-size:0.85rem}
  </style>
</head>
<body>
  <h1>GlobalCampaignHub Blog — ${langMeta.name}</h1>
  ${cards}
  <p><a href="${BASE_URL}/" style="color:#47ffb8">← Back to Hub</a></p>
</body>
</html>`, 'utf-8');
}
...
async function main() {
  ...
  const langOverride = args.indexOf('--lang') !== -1
  ...
  for (const kw of keywords) {
    ...
    if (!post) {
      console.error(`❌ Skipping "${kw.keyword}" — all attempts failed`);
      process.exit(0); // exit cleanly so workflow continues
    }
...
main().catch(err => {
  ...
  process.exit(0); // exit 0 so workflow doesn't fail hard
});
