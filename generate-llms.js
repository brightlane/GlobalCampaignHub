/**
 * generate-llms.js — GlobalCampaignHub
 * ──────────────────────────────────────
...
for (const kw of RAW_KEYWORDS) {
  ...
  if (!merchants[m]) {
    merchants[m] = { cat: kw.cat, lc: kw.lc, keywords: [] };
  }
  merchants[m].keywords.push(kw.keyword);
}

// Collect published blog pages
...
function buildLlms() {
  ...
  const topKeywords = RAW_KEYWORDS.slice(0, 50).map(k => `- ${k.keyword} (vol: ${k.volume.toLocaleString()})`).join('\n');
